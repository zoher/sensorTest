...
LINKER_FLAGS=-lm -mthumb -mhard-float -mcpu=cortex-m4 -mfpu=fpv4-sp-d16 -Wl,--gc-sections -T$(LINK_SCRIPT) -static  -Wl,-cref "-Wl,-Map=$(BIN_DIR)/sensorTest.map" -Wl,--defsym=malloc_getpagesize_P=0x1000
...
buildelf: $(OBJS) 
	$(CC) -o "$(BIN_DIR)/sensorTest.elf" $(OBJS) $(LINKER_FLAGS)

clean:
	$(RM) $(OBJS) "$(BIN_DIR)/sensorTest.elf" "$(BIN_DIR)/sensorTest.map"
